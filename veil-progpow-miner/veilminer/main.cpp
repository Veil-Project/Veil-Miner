...
bool g_exitOnError = false;  // Whether or not veilminer should exit on mining threads errors
...
#undef BACKTRACE_MAX_FRAMES
    ...
    bool validateArgs(int argc, char** argv)
    {
        ...
        CLI::App app("veilminer - GPU ProgPOW(0.9.3) miner for Zing");
    ...
    void help()
    {
        cout << "veilminer - GPU ProgPOW(0.9.3) miner for Zing" << endl
             << "minimal usage : veilminer [DEVICES_TYPE] [OPTIONS] -P... [-P...]" << endl
             << endl
             << "Devices type options :" << endl
             << endl
             << "    By default veilminer will try to use all devices types" << endl
    ...
    void helpExt(std::string ctx)
    {
        ...
        if (ctx == "api")
        {
            cout << "API Interface Options :" << endl
                 << endl
                 << "    veilminer provide an interface for monitor and or control" << endl
        ...
        if (ctx == "misc")
        {
            cout << "Miscellaneous Options :" << endl
                 ...
                 << "                        Sets the number of minutes veilminer can stay" << endl
                 ...
                 << "    --exit              FLAG Stop veilminer whenever an error is encountered"
                 << endl
                 << "    --ergodicity        INT[0 .. 2] Default = 0" << endl
                 << "                        Sets how veilminer chooses the nonces segments to"
                 << endl
                 << "                        search on." << endl
        ...
        if (ctx == "con")
        {
            cout << "Connections specifications :" << endl
                 ...
                 << "    the fail-over disconnects veilminer passes to the next connection" << endl
                 << "    available and so on till the list is exhausted. At that moment" << endl
                 << "    veilminer restarts the connection cycle from the first one." << endl
                 << "    An exception to this behavior is ruled by the --failover-timeout" << endl
                 << "    command line argument. See 'veilminer -H misc' for details." << endl
                 << endl
                 << "    The special notation '-P exit' stops the failover loop." << endl
                 << "    When veilminer reaches this kind of connection it simply quits." << endl
                 << endl
                 << "    When using stratum mode veilminer tries to auto-detect the correct" << endl
...
int main(int argc, char** argv)
{
    ...
    auto* bi = veilminer_get_buildinfo();
    cout << endl
         << endl
         << "veilminer " << bi->project_version << endl
    ...
    if (argc < 2)
    {
        cerr << "No arguments specified. " << endl
             << "Try 'veilminer --help' to get a list of arguments." << endl
             << endl;
        return 1;
    }
    ...
    try
    {
        ...
        catch (std::invalid_argument& ex1)
        {
            cerr << "Error: " << ex1.what() << endl
                 << "Try veilminer --help to get an explained list of arguments." << endl
                 << endl;
            return 1;
        }
